...
class pasteNmergeDialog(QDialog):
    ...
    def __init__(self, overwrite, ltitle="This action will overwrite some items.", parent=None):
        ...
        vbox.addWidget(label1)
        # #
        # self.dcbox = QCheckBox("Make the default choise")
        # vbox.addWidget(self.dcbox)
        #
        hbox = QBoxLayout(QBoxLayout.LeftToRight)
        vbox.addLayout(hbox)
        # skip all the items
    ...
    def getValue(self):
        #return (self.Value,self.ischecked)
        return self.Value
    
    def fsetValue(self, n):
        self.Value = n
        # #self.ischecked = self.dcbox.isChecked()
        # # 0 no - 2 yes
        # self.ischecked = self.dcbox.checkState()
        self.close()
    ...
    def dropEvent(self, event):
        ...
        if event.mimeData().hasUrls:
            ...
            if filePaths:
                ...
                if pointedItem.isValid():
                    ifp = self.model().data(pointedItem, QFileSystemModel.FilePathRole)
                    # proposedAction: 1 copy - 2 cut - 4 link
                    if os.path.isdir(ifp):
